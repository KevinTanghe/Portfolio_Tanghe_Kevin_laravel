...
@section('content')
    <div class="container">
        <table class="table">
            ...
            <tbody>
                @foreach ($folioTitle as $item)
                    <tr>
                        <th scope="row">{{ $item->id }}</th>
                        <td>{{ $item->ombreTitle }}</td>
                        <td>{{ $item->title }}</td>
                        <td>{{ $item->sousTitle }}</td>
                        <td><a class="btn btn-success" href="/edit-portfolioTitle/{{$item->id}}">Edit</a></td>
                        <td><a class="btn btn-danger" href="/delete-portfolioTitle/{{$item->id}}">Delete</a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        @endif
        <form action="/store-portfolioTitle" method="POST">
            @csrf
